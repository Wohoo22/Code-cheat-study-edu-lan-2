...
$.ajax({
    type: "GET",
    cache: false,
    url: "/home/scoredetails?id=" + id,
    async: false,
    success: function (data) {
        div = document.createElement("div")
        div.insertAdjacentHTML( 'beforeend', data );      
        dapAn = div.getElementsByClassName("text-success");
        for(i=2;i<dapAn.length;i+=3){
    		var cauHoi =  dapAn[i-1].parentNode.nextElementSibling.lastElementChild.innerText.replace(/(\n| |\$|\t)/gm, "");
		var answer =  dapAn[i].parentNode.nextElementSibling.lastElementChild.innerText.replace(/(\n| |\$|\t)/gm, "");
	...
	dapAnSai = div.getElementsByClassName("text-danger");
	for(i=3;i<dapAnSai.length;i+=3){
        	var cauHoi =  dapAnSai[i-1].parentNode.nextElementSibling.lastElementChild.innerText.replace(/(\n| |\$|\t)/gm, "");
		var answer =  dapAnSai[i].parentNode.nextElementSibling.lastElementChild.replace(/(\n| |\$|\t)/gm, "");
